...
namespace {
  ...
  TString minht("400");
  TString minbm("1");
  TString luminosity="0.016"; // in ifb
...
int main(){ 
  ...
  TString folder_nsdata = "/cms24r0/jaehyeok/susy_cfa_babies/2015_09_25/";
  TString folder_1l     = "/cms24r0/jaehyeok/susy_cfa_babies/2015_09_28/skim_1vlht400/";
  TString folder_1ldata = "/cms24r0/jaehyeok/susy_cfa_babies/2015_09_25/";
  TString folder_2l     = "/cms24r0/jaehyeok/susy_cfa_babies/2015_09_28/skim_2vl/";
  TString folder_2ldata = "/cms24r0/jaehyeok/susy_cfa_babies/2015_09_25/";
  ...
  s_tt_ns.push_back(folder_2l+"*TTJets_SingleLeptFromT*25ns*");
  s_tt_ns.push_back(folder_2l+"*TTJets_DiLept*25ns*");
  ...
  //Samples.push_back(sfeats(s_trig_htmht, datatitle,kBlack,1,"(trig[4]||trig[8])&&json_golden")); Samples.back().isData = true;
  ...
  Samples.push_back(sfeats(s_trig_htmht, "Data 2l, N_{jets} #geq 4",kBlack,1,
			   ...
			   //"(trig[4]||trig[8])&&njets>=4&&(nvmus+nvels)==2&&json_golden")); Samples.back().isData = true;
  Samples.push_back(sfeats(s_trig_htmht, "Data 1l, N_{jets} #geq 5, m_{T}#leq140, n_{b}#geq1",dps::c_tt_1l,1,
			   ...
			   //"(trig[4]||trig[8])&&njets>=5&&(nvmus+nvels)==1&&mt<=140&&nbm>=1&&json_golden")); //Samples.back().isData = true;
  vector<int> mj_sam;
  ...
  vars.push_back(hfeats("fjets_m[0]",8,0,480, ra4_sam, "m(J_{1}) [GeV]",
  			"pass&&onht>350&&onmet>100&&(nvels+nvmus)==1&&ht>"+minht+
 ...
 */  
  ...
  vars.push_back(hfeats("Max$(mus_mt*(mus_miniso<0.2))+Max$(els_mt*(els_miniso<0.1))",18,0,360, ra4_sam, "m_{T} (veto lepton) [GeV]",
          "pass&&onht>350&&onmet>100&&((nvels==1 && nvmus==0)||(nvels==0&&nvmus==1))&&ht>"+minht+"&&"+metcut+"&&njets>="+minjets+"&&nbm>="+minbm));
  vars.back().whichPlots = "1"; vars.back().normalize = true;

  vars.push_back(hfeats("Max$(mus_mt*(mus_miniso<0.2))",18,0,360, ra4_sam, "m_{T} (veto muon) [GeV]",
          "pass&&onht>350&&onmet>100&&(nvels==0&&nvmus==1)&&ht>"+minht+"&&"+metcut+"&&njets>="+minjets+"&&nbm>="+minbm));
  vars.back().whichPlots = "1"; vars.back().normalize = true;
  
  vars.push_back(hfeats("Max$(els_mt*(els_miniso<0.1))",18,0,360, ra4_sam, "m_{T} (veto electron) [GeV]",
          "pass&&onht>350&&onmet>100&&(nvels==1&&nvmus==0)&&ht>"+minht+"&&"+metcut+"&&njets>="+minjets+"&&nbm>="+minbm));
  vars.back().whichPlots = "1"; vars.back().normalize = true;
  
  vars.push_back(hfeats("mt",18,0,360, ra4_sam, "m_{T} [GeV]",
          "pass&&onht>350&&onmet>100&&(nvels+nvmus)==1&&ht>"+minht+"&&"+metcut+"&&njets>="+minjets+"&&nbm>="+minbm));
  vars.back().whichPlots = "1"; vars.back().normalize = true;

  vars.push_back(hfeats("met",16,0,800, ra4_sam, "MET [GeV]",
          "pass&&onht>350&&onmet>100&&(nvels+nvmus)==1&&ht>"+minht+"&&njets>="+minjets+"&&nbm>="+minbm));
  vars.back().whichPlots = "1"; vars.back().normalize = true;
  ...
  vars.push_back(hfeats("njets",11,-0.5,10.5, ra4_sam, "Number of jets",
          "pass&&onht>350&&onmet>100&&(nvels+nvmus)==1&&ht>"+minht+"&&"+metcut+"&&nbm>="+minbm));
  vars.back().whichPlots = "2"; vars.back().normalize = true;

  vars.push_back(hfeats("nbm",5,-0.5,4.5, ra4_sam, "Number of b-tags (CSVM)",
          "pass&&onht>350&&onmet>100&&(nvels+nvmus)==1&&ht>"+minht+"&&"+metcut+"&&njets>="+minjets));
  vars.back().whichPlots = "2"; vars.back().normalize = true;
  
  vars.push_back(hfeats("nfjets",8,-0.5,7.5, ra4_sam, "Number of fatjets",
          "pass&&onht>350&&onmet>100&&(nvels+nvmus)==1&&ht>"+minht+"&&"+metcut+"&&njets>=4&&nbm>="+minbm));
  vars.back().whichPlots = "2"; vars.back().normalize = true;

  vars.push_back(hfeats("jets_pt[0]",16,0,800, ra4_sam, "p_{T}(j_{1}) [GeV]",
          "pass&&onht>350&&onmet>100&&(nvels+nvmus)==1&&ht>"+minht+"&&"+metcut+"&&njets>=4&&nbm>="+minbm+"&&jets_islep[0]==0"));
  vars.back().whichPlots = "2";  vars.back().normalize = true;
  
  vars.push_back(hfeats("jets_pt",16,0,800, ra4_sam, "p_{T}(j) [GeV]",
          "pass&&onht>350&&onmet>100&&(nvels+nvmus)==1&&ht>"+minht+"&&"+metcut+"&&njets>=4&&nbm>="+minbm+"&&jets_islep==0"));
  vars.back().whichPlots = "2";  vars.back().normalize = true;
  ...
  vars.push_back(hfeats("els_pt",10,0,200, ra4_sam, "p_{T}(veto electron) [GeV]",
          "pass&&onht>350&&onmet>100&&(nvels==1&&nvmus==0)&&els_miniso<0.1&&ht>"+minht+"&&"+metcut+"&&njets>=4&&nbm>="+minbm));
  vars.push_back(hfeats("els_pt",10,0,200, ra4_sam, "p_{T}(veto electron) [GeV]",
          "pass&&onht>350&&onmet>100&&(nvels==1&&nvmus==0)&&els_miniso<0.1&&ht>"+minht+"&&"+metcut+"&&njets>=4&&nbm>="+minbm));
  vars.back().whichPlots = "2";  vars.back().normalize = true;
  
  vars.push_back(hfeats("mus_pt",10,0,200, ra4_sam, "p_{T}(veto muon) [GeV]",
          "pass&&onht>350&&onmet>100&&(nvels==0&&nvmus==1)&&mus_miniso<0.2&&ht>"+minht+"&&"+metcut+"&&njets>=4&&nbm>="+minbm));
  vars.back().whichPlots = "2";  vars.back().normalize = true;
...
/*
  ...
  // // MJ distributions
   ...
   vars.back().whichPlots = "2"; // vars.back().normalize = true;
*/
  plot_distributions(Samples, vars, luminosity, plot_type, plot_style, "1d_2015c",true);

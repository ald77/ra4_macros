...
int main(){ 
  ...
  */
  ...
  vars.push_back(hfeats("tks_mt",30,0,300, ra4_tt_t1, "track m_{T} (GeV), electron tracks, chg+neu iso < 0.2",
			     "ht>500&&mj>300&&met>200&&nbm>=1&&mt>140&&njets>=6&&(nmus+nels)==1&&((tks_id*lep_charge)>0)&&(!tks_is_primary)&&tks_tru_id*tks_tru_id==121&&tks_from_w&&(tks_mini_ne+tks_mini_ch)<0.2"));
    vars.push_back(hfeats("tks_mt",30,0,300, ra4_tt_t1, "track m_{T} (GeV), electron tracks, chg+neu iso < 1.0",
			     "ht>500&&mj>300&&met>200&&nbm>=1&&mt>140&&njets>=6&&(nmus+nels)==1&&((tks_id*lep_charge)>0)&&(!tks_is_primary)&&tks_tru_id*tks_tru_id==121&&tks_from_w&&(tks_mini_ne+tks_mini_ch)<1.0"));
  

       vars.push_back(hfeats("tks_mini_ne+tks_mini_ch",60,0,6, ra4_tt_t1, "chg+neu mini isolation, electron tracks",
			     "ht>500&&mj>300&&met>200&&nbm>=1&&mt>140&&njets>=6&&(nmus+nels)==1&&((tks_id*lep_charge)>0)&&(!tks_is_primary)&&tks_tru_id*tks_tru_id==121&&tks_from_w"));
       ...
       vars.push_back(hfeats("tks_mini_ne+tks_mini_ch",60,0,6, ra4_tt_t1, "chg+neu mini isolation, muon tracks",
			     "ht>500&&mj>300&&met>200&&nbm>=1&&mt>140&&njets>=6&&(nmus+nels)==1&&((tks_id*lep_charge)>0)&&(!tks_is_primary)&&tks_tru_id*tks_tru_id==169&&tks_from_w"));
            vars.push_back(hfeats("tks_mini_ne+tks_mini_ch",60,0,6, ra4_tt_t1, "chg+neu mini isolation, muon tracks",
			     "ht>500&&mj>300&&met>200&&nbm>=1&&mt>140&&njets>=6&&(nmus+nels)==1&&((tks_id*lep_charge)>0)&&(!tks_is_primary)&&tks_tru_id*tks_tru_id==169&&!tks_from_w"));
       
       vars.push_back(hfeats("tks_mini_ne+tks_mini_ch",60,0,6, ra4_tt_t1, "chg+neu mini isolation, hadronic tracks truth-matched",
			     "ht>500&&mj>300&&met>200&&nbm>=1&&mt>140&&njets>=6&&(nmus+nels)==1&&((tks_id*lep_charge)>0)&&(!tks_is_primary)&&!(tks_tru_id*tks_tru_id==121||tks_tru_id*tks_tru_id==169)&&tks_from_w&&tks_from_tauhad"));
       ...
       vars.push_back(hfeats("tks_mini_ne+tks_mini_ch",60,0,6, ra4_tt_t1, "chg+neu mini isolation, hadronic tracks truth-matched",
			     "ht>500&&mj>300&&met>200&&nbm>=1&&mt>140&&njets>=6&&(nmus+nels)==1&&((tks_id*lep_charge)>0)&&(!tks_is_primary)&&!(tks_tru_id*tks_tru_id==121||tks_tru_id*tks_tru_id==169)&&tks_from_w&&tks_mt<100&&tks_from_tauhad"));
  ...
  for(unsigned var(0); var<vars.size(); var++){
    ...
    for(unsigned sam(0); sam < vars[var].samples.size(); sam++){
      ...
      if(var==0) nevents.push_back(nentries[sam]);
	...
	histo[1][var][sam]->Draw();
      ...
      if(vars[var].title.Contains("mini") && sam !=1){
	
	cout<<Samples[isam].label<<" track isolation efficiency"<<endl<<Form("0.2: %.f",histo[1][var][sam]->Integral(histo[1][var][sam]->FindBin(0),histo[1][var][sam]->FindBin(0.2)))<<"%"<<endl;
	cout<<Form("0.3: %.f",histo[1][var][sam]->Integral(histo[1][var][sam]->FindBin(0),histo[1][var][sam]->FindBin(0.3)))<<"%"<<endl;
	cout<<Form("0.5: %.f",histo[1][var][sam]->Integral(histo[1][var][sam]->FindBin(0),histo[1][var][sam]->FindBin(0.5)))<<"%"<<endl;
	cout<<Form("1.0: %.f",histo[1][var][sam]->Integral(histo[1][var][sam]->FindBin(0),histo[1][var][sam]->FindBin(1.0)))<<"%"<<endl;
	cout<<Form("1.5: %.f",histo[1][var][sam]->Integral(histo[1][var][sam]->FindBin(0),histo[1][var][sam]->FindBin(1.5)))<<"%"<<endl<<endl;
	
	/*TLatex *lab1 = new TLatex(0.2,0.2,"Track Efficiency");
        lab1->SetNDC();
        lab1->SetTextSize(0.05);
        lab1->SetLineWidth(2);
		TLatex *lab2 = new TLatex(0.2,0.2,"Track Efficiency");
        lab2->SetNDC();
        lab2->SetTextSize(0.05);
        lab2->SetLineWidth(2);
		TLatex *lab3 = new TLatex(0.2,0.2,"Track Efficiency");
        lab3->SetNDC();
        lab3->SetTextSize(0.05);
        lab3->SetLineWidth(2);*/
      ...
      leghisto+= +" [N_{tks} = " + RoundNumber(nentries[sam],0) + ", from N_{events} = "+RoundNumber(nevents[sam],0)+"]";
    ...
    can.SetLogy(0);
    pname = "plots/1d5/shapes_"+vars[var].tag+".eps";
    can.SaveAs(pname);
    histo[1][var][0]->SetMaximum(maxhisto*maxLog);
    can.SetLogy(1);
    pname = "plots/1d5/log_shapes_"+vars[var].tag+".eps";
    //can.SaveAs(pname);
  }// Loop over variables

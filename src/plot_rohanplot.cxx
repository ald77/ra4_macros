...
int main(){ 
  ...
  vars.push_back(hfeats("mj",30,0,1500, mt140,    "M_{J} [GeV]",baseline,400,"eventselection")); 
  vars.back().whichPlots = "14"; vars.back().maxRatio = 0.29;
  ...
  // vars.push_back(hfeats("njets",12,0.5,12.5, mt140, "N_{jets}",baseline, 5.5, "allj_12")); vars.back().maxRatio = 0.29;
  // vars.back().whichPlots = "13"; 
  vars.push_back(hfeats("njets",7,5.5,12.5, mt140, "N_{jets}",baseline, -1, "eventselection")); 
  vars.back().whichPlots = "13"; vars.back().maxRatio = 0.29;

  //NOTE: Make sure RohanHack is turned off
  plot_distributions(Samples, vars, "3", ".pdf", "CMSPaper","rohan",true);
}

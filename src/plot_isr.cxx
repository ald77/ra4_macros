...
namespace {
  TString luminosity="0.135"; // in ifb
...
int main(){ 
  TString folder_1l="/net/cms2/cms2r0/babymaker/babies/2015_10_19/mc/skim_1lht400mc/";
  TString folder_2vl="/net/cms2/cms2r0/babymaker/babies/2015_10_19/mc/skim_1lht400mc/";
  ...
  TString folder_nsdata="/net/cms2/cms2r0/babymaker/babies/2015_10_19/data/singlelep/combined/";
  TString folder_2ldata="/net/cms2/cms2r0/babymaker/babies/2015_10_19/data/doublelep/";
  ...
  s_other.push_back(folder_1l+"*_DYJetsToLL_M-50_HT*");
  ...
  //Samples.push_back(sfeats(s_trig_htmht, datatitle,kBlack,1,"trig[0]&&nonblind")); Samples.back().isData = true;
  Samples.push_back(sfeats(s_trig_sl, datatitle,kBlack,1,"(trig[4]||trig[8])&&nonblind")); Samples.back().isData = true;
  ...
  Samples.push_back(sfeats(s_trig_sl, "Data 2l, N_{jets} #geq 4",kBlack,1,
			   "(trig[4]||trig[8])&&njets>=4&&(nmus+nels)==2&&nonblind")); Samples.back().isData = true;
  Samples.push_back(sfeats(s_trig_sl, "Data 1l, N_{jets} #geq 5, m_{T}#leq140, n_{b}#geq1",dps::c_tt_1l,1,
			   "(trig[4]||trig[8])&&njets>=5&&(nmus+nels)==1&&mt<=140&&nbm>=1&&nonblind")); //Samples.back().isData = true;
  vector<int> mj_sam;
  ...
  // 			   "(trig[10]||trig[9])&&nonblind")); Samples.back().isData = true;
  // Samples.push_back(sfeats(s_trig_sl, "Data",kBlack,1,
		// 	   "(trig[18]||trig[22])&&nonblind")); Samples.back().isData = true;
  Samples.push_back(sfeats(s_DY, "Z+jets", dps::c_qcd));//12
  ...
  vector<int> dmu_sam(0), del_sam(0);
  vector<sfeats> Samples2; 
  Samples2.push_back(sfeats(s_trig_dmu, "Data",kBlack,1,
			   "(trig[9])&&nonblind")); Samples2.back().isData = true;
  dmu_sam.push_back(Samples2.size()-1);
  Samples2.push_back(sfeats(s_trig_del, "Data",kBlack,1,
			   "(trig[10])&&nonblind")); Samples2.back().isData = true;
  del_sam.push_back(Samples2.size()-1);
  Samples2.push_back(sfeats(s_DY, "Z+jets", dps::c_qcd));//12
  dmu_sam.push_back(Samples2.size()-1);
  del_sam.push_back(Samples2.size()-1);
  Samples2.push_back(sfeats(s_tt_dl, "t#bar{t}, 2 true leptons", dps::c_tt_2l,1,"ntruleps>=2"));
  dmu_sam.push_back(Samples2.size()-1);
  del_sam.push_back(Samples2.size()-1);
  Samples2.push_back(sfeats(s_tt_dl, "t#bar{t}, 1 true lepton", dps::c_tt_1l, 1,"ntruleps==1"));
  dmu_sam.push_back(Samples2.size()-1);
  del_sam.push_back(Samples2.size()-1);
  ...
  //  			"(nvmus>=2||nvels>=2)&&njets>=4&&"+mlvlv));
  // vars.back().whichPlots = "2"; // vars.back().normalize = true;
  
  // vars.push_back(hfeats("njets",9,-0.5,8.5, dl_sam, "N_{jets} [GeV]",
  //       "ht>350&&(nvmus>=2||nvels>=2)&&"+mlvlv));
  // vars.back().whichPlots = "2"; vars.back().normalize = true;

  // vars.push_back(hfeats("njets",9,-0.5,8.5, dmu_sam, "N_{jets} [GeV]",
  //  			"ht>350&&(nvmus>=2)&&mumuv_m>80&&mumuv_m<100"));
  // vars.back().whichPlots = "2"; vars.back().normalize = true;

  // vars.push_back(hfeats("njets",9,-0.5,8.5, del_sam, "N_{jets} [GeV]",
  //       "ht>350&&(nvels>=2)&&elelv_m>80&&elelv_m<100"));
  // vars.back().whichPlots = "2"; vars.back().normalize = true;
  ...
  //  			"ht>350&&(nvmus>=2||nvels>=2)&&njets>=4&&"+mlvlv));
  // vars.back().whichPlots = "2"; // vars.back().normalize = true;
  ...
  //  			"ht>350&&(nvmus>=2||nvels>=2)&&njets<=3&&"+mlvlv));
  // vars.back().whichPlots = "2"; // vars.back().normalize = true;
  
  // vars.push_back(hfeats("mj",20,0,600, dmu_sam, "M_{J} [GeV]",
  //  			"ht>350&&(nvmus>=2)&&njets>=4&&mumuv_m>80&&mumuv_m<100"));
  // vars.back().whichPlots = "2"; vars.back().normalize = true;
  
  // ZpT : elel and mumu separately
  
  // (nvmus>=2||nvels>=2)
  // vars.push_back(hfeats("mumuv_pt+elelv_zpt*(elelv_zpt>0)",10,0,500, dl_sam, "p_{T}(#font[12]{ll}) [GeV]",
  //  			"ht>350&&(nvmus>=2||nvels>=2)&&"+mlvlv));
  // vars.back().whichPlots = "2"; // vars.back().normalize = true;
  
  // vars.push_back(hfeats("mumuv_pt*(mumuv_pt>0)+elelv_zpt*(elelv_zpt>0)",10,0,500, dl_sam, "p_{T}(#font[12]{ll}) [GeV]",
  //  			"ht>350&&(nvmus>=2||nvels>=2)&&njets>=4&&"+mlvlv));
  // vars.back().whichPlots = "2"; // vars.back().normalize = true;
  
  // // (nvels>=2) 
  // vars.push_back(hfeats("elelv_zpt*(elelv_zpt>0)",10,0,500, dl_sam, "p_{T}(#font[12]{ee}) [GeV]",
  //  			"ht>350&&(nvels>=2)&&"+mlvlv));
  // vars.back().whichPlots = "2"; // vars.back().normalize = true;
  
  // vars.push_back(hfeats("elelv_zpt*(elelv_zpt>0)",10,0,500, dl_sam, "p_{T}(#font[12]{ee}) [GeV]",
  //  			"ht>350&&(nvels>=2)&&njets>=4&&"+mlvlv));
  // vars.back().whichPlots = "2"; // vars.back().normalize = true;
  
  // ------------- DILEPTON TRIGGER
  // vars.push_back(hfeats("mumuv_pt*(mumuv_pt>0)",10,0,500, dmu_sam, "p_{T}(#mu#mu) [GeV]",
  //  			"ht>350&&(nvmus>=2)&&mumuv_m>80&&mumuv_m<100"));
  // vars.back().whichPlots = "2"; vars.back().normalize = true;
  ...
  // vars.push_back(hfeats("mumu_pt*(mumu_pt>0)",10,0,500, dmu_sam, "p_{T}(#mu#mu) [GeV]",
  //       "ht>350&&(nmus>=2)&&mumuv_m>80&&mumuv_m<100"));
  // vars.back().whichPlots = "2"; vars.back().normalize = true;
  
  // vars.push_back(hfeats("mumu_pt*(mumu_pt>0)",10,0,500, dmu_sam, "p_{T}(#mu#mu) [GeV]",
  //       "ht>350&&(nmus>=2)&&njets>=4&&mumuv_m>80&&mumuv_m<100"));
  // vars.back().whichPlots = "2"; vars.back().normalize = true;
  
  //Single lepton trigger
  // vars.push_back(hfeats("mumuv_pt",10,0,500, ra4_sam, "p_{T}(#mu#mu) [GeV]",
  //       "ht>350&&nvmus==2&&nmus>=1&&mumuv_m>80&&mumuv_m<100"));
  // vars.back().whichPlots = "2"; vars.back().normalize = true;

  // vars.push_back(hfeats("elelv_pt",10,0,500, ra4_sam, "p_{T}(ee) [GeV]",
  //       "ht>350&&nvels==2&&nels>=1&&elelv_m>80&&elelv_m<100"));
  // vars.back().whichPlots = "2"; vars.back().normalize = true;
  ...
  vars.push_back(hfeats("jetsys_nob_pt",12,0,600, ra4_sam, "p_{T}(ISR jets) [GeV]",
        "ht>350&&nels==1&&nmus==1&&njets>2&&nbm==2"));
  vars.back().whichPlots = "2"; vars.back().normalize = true;

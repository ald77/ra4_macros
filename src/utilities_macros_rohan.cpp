//----------------------------------------------------------------------------
// utilities_macros - Various functions used accross thecode
//----------------------------------------------------------------------------

#ifndef INT_ROOT
#include "utilities_macros_rohan.hpp"
#endif
...
void plot_distributions(vector<sfeats> Samples, vector<hfeats> vars, TString luminosity, 
  ...
  for(unsigned var(0); var<vars.size(); var++){
    ...
    for(unsigned sam(Nsam-1); sam < Nsam; sam--){
      ...
      if(rohanHack && (sam==0||sam==1)) {histo[0][var][sam]->Add(histo[0][var][sam+2]);}
	...
	for(unsigned sam(Nsam-1); sam < Nsam; sam--) {
	  ...
	  hratio_mcscale = static_cast<TH1D*>(histo[0][var][firstplotted]->Clone());  
	  //	  hratio_data->Scale(100./hratio_data->Integral());
	  //	  hratio_mcscale->Scale(100./hratio_mcscale->Integral());
	...
	TString lumilbl = TString::Format("L = %1.f",luminosity.Atof())+" fb^{-1}";
	TLatex llbl;
...
TString cuts2title(TString title){
  ...
  title.ReplaceAll("ht>500&&met>200&&njets>=7&&nbm>=2&&nleps==1","Baseline");
  title.ReplaceAll("ht>500&&met>200&&njets>=7&&nbm>=1&&nleps==1","BaselineNb1");
...
void hfeats::format_tag(){
  ...
  tag.ReplaceAll("htg500_metg200_njetsge7_nbmge1_nleps1_","Baseline1B_");
  tag.ReplaceAll("htg500_metg200_njetsge7_nbmge2_nleps1_","Baseline_");
